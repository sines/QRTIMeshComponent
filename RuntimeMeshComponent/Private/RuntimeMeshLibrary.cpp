...
// define AXIS_COLOR
#define AXIS_COLOR_X FLinearColor(0.594f,0.0197f,0.0f)
#define AXIS_COLOR_Y FLinearColor(0.1349f,0.3959f,0.0f)
#define AXIS_COLOR_Z FLinearColor(0.1349f,0.3959f,0.0f)
...
void URuntimeMeshLibrary::BuildConeVerts(float Angle1, float Angle2, float Scale, float XOffset, int32 NumSides, TArray<FVector>& Vertices, TArray<int32>& OutIndices)
{
	...
	for (auto It = OutVerts.CreateConstIterator(); It; ++It)
	{
		Vertices.Add(It->Position);
	}
};

void URuntimeMeshLibrary::BuildCylinderMesh(const FVector& Base, const FVector& XAxis, const FVector& YAxis, const FVector& ZAxis, float Radius, float HalfHeight, int32 Sides, TArray<FVector>& Vertices, TArray<int32>& OutIndices)
{
	TArray<FDynamicMeshVertex> OutVerts;
	// Build CylinderMesh
	...
	//UGameplayStatics::GetAllActorsOfClass(, classToFind, foundEnemies);
};

void URuntimeMeshLibrary::BuildAxis(const FVector& InLocation, bool bDrawWidget)
{
	// Figure out axis colors
	const FLinearColor& XColor = AXIS_COLOR_X;
	const FLinearColor& YColor = AXIS_COLOR_Y;
	const FLinearColor& ZColor = AXIS_COLOR_Z;
	...
	AxisMaterialX->SetVectorParameterValue("GizmoColor", AXIS_COLOR_X);

	AxisMaterialY = UMaterialInstanceDynamic::Create(AxisMaterialBase, NULL);
	AxisMaterialY->SetVectorParameterValue("GizmoColor", AXIS_COLOR_Y);

	AxisMaterialZ = UMaterialInstanceDynamic::Create(AxisMaterialBase, NULL);
	AxisMaterialZ->SetVectorParameterValue("GizmoColor", AXIS_COLOR_Z);

	XAxisDir = FVector2D::ZeroVector;

	FMatrix AxisRotation = FMatrix::Identity;
	FMatrix ArrowToWorld = AxisRotation * WidgetMatrix;
}
